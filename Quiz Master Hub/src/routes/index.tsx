...
import { Brain, PenLine, Trophy, ArrowRight } from "lucide-react";
...
function Index() {
  ...
  function handleStep(step: (typeof steps)[number]) {
    if (step.requiresAuth && !loading && !user) {
      toast.error("Sign in first to " + step.cta.toLowerCase() + ".");
      navigate({ to: "/auth", search: { redirect: step.to } });
      return;
    }
    navigate({ to: step.to });
  }
        ...
        <div className="mx-auto grid max-w-5xl gap-5 sm:grid-cols-3">
          {steps.map((step) => (
            <button
              ...
              disabled={loading}
              className="card-pop group p-6 text-left transition-transform hover:-translate-y-1 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:opacity-70"
            >
